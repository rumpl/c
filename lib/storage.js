...
storage.list = function (root) {
  return _.filter(fs.readdirSync(root), function (el) {
    return el !== storage.STORE;
  });
};
...
storage.loadComments = function (node) {
  ...
  files.forEach(function (el, idx) {
    ret[path.basename(el, storage.POSTFIX)] = fs
      .readFileSync(path.join(node, storage.STORE, el))
      .toString();
  });
